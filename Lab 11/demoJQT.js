...
//$("") will be used as a frame of reference
   ...
   //Lab Eleven
   //Next()
   $("#button7").on("click",function(){//works
      $("#content").next().css("background-color","yellow");
   });

   //prev()
   $("#button8").on("click",function(){///does not work?
      $("#content").prev().css("background-color","grey");
   });

   //closet()
   $("#button9").on("click",function(){//works
      $("#item1").closest('ul').css("color","orange");
   });

  //first()
   $("#button10").on("click",function(){ //works
      $("li").first().css("background-color","red");
   });

   //last()
   $("#button11").on("click",function(){//works
       $("li").last().css("background-color","red");
   });

   //filter()
   $("#button12").on("click",function(){//works
      $("li").filter('#item2').css("background-color","red");
  });

  //not()
  $("#button13").on("click",function(){//works
   $("li").not('#item2').css("background-color","red");
  });

  //slice()
  $("#button14").on("click",function(){//works
